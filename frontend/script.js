        ...
        if (response && (response._id || response.dayId)) {
          ...
          if (item.localId && item.localId !== mainId) {
            ...
            if (item.entity === 'days') {
              ...
              // Also update any offline scratchpads mapped to this temp ID
  ...
  if (currentPermission === 'granted') {
    try {
      ...
      if (!manuallyDisabled) {
        ...
        await navigator.serviceWorker.register('/sw.js?v=43');
...
async function openScratchpad(dayId) {
  ...
  if (navigator.onLine && !String(dayId).startsWith('temp_') && !hasPendingSync && !data) {
    try {
      const response = await apiFetch(`${API}/api/days/${dayId}/scratchpad`);
      ...
      console.warn('Failed to fetch scratchpad from API, using cache:', err);
    }
  }
...
async function saveScratchpadDrawing() {
  ...
  const day = allDays.find(d => d._id === scratchpadDayId);
  if (day) {
  ...
  if (navigator.onLine && !String(scratchpadDayId).startsWith('temp_')) {
    try {
      await apiFetch(`${API}/api/days/${scratchpadDayId}/scratchpad`, {
        method: 'PUT',
      ...
      syncManager.addToQueue('PUT', 'scratchpads', scratchpadDayId, { strokes: scratchpadStrokes });
